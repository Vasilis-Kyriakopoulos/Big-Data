...
public class NewWordCount {
	...
	public static String readFileFromHDFS() throws IOException {
        ...
        Path hdfsReadPath = new Path("/r_output2.1/" + fileName);
	...
	public static void main(String[] args) throws Exception {
		...
		FileOutputFormat.setOutputPath(job1,new Path("/r_output2.1"));
		
		
		job1.waitForCompletion(true) ;
		
		
	    //String s = "303.9357142857143";
		
		Configuration conf = new Configuration();
		String s = readFileFromHDFS();
		conf.set("Mean", s);
		...
		FileOutputFormat.setOutputPath(job,new Path("/r_output2.2"));
		...
		Job job3 = Job.getInstance(conf3, "count2");
		...
		job3.setMapperClass(NewWordMapper2.class);

		//job.setCombinerClass(NewWordReducer.class);
		job3.setReducerClass(NewWordReducer2.class);

		FileInputFormat.addInputPath(job3, new Path("/r_output2.2"+"/part-r-00000"));

		FileOutputFormat.setOutputPath(job3,new Path("/r_output2.3"));
